<template>
  <div class="flex align-middle w-fit h-fit justify-center items-center transition-all">
    <div :data-size="props.size" :class="[
      'flex flex-col bg-slate-800/[0.6] backdrop-blur-lg rounded-lg',
      ...
      <div v-if="labelTop !== ''" :data-size="props.size" :class="[
      ...
      <div :data-size="props.size" :class="[
        ...
        <div v-if="labelLeft !== ''" :data-size="props.size" :class="[
        ...
        <div v-if="timeArr[0][0] !== undefined" :data-size="props.size" :key="lengthArr[0]" :class="[
          ...
          <div class="animate-flip-clock " v-for="(timeStr, index) in timeArr[0]" :key="`${index}-${timeStr}`">
            {{ timeStr }}
          </div>
        </div>
        <div v-if="timeArr[1][0] !== undefined" :data-size="props.size" :key="lengthArr[1]" :class="[
          'flex flex-col items-center overflow-y-hidden leading-none',
          `w-[${lengthArr[1]}ch]`,
          ...
          <div class="animate-flip-clock" v-for="(timeStr, index) in timeArr[1]" :key="`${index}-${timeStr}`">
            {{ timeStr }}
          </div>
        </div>
        <div v-if="timeArr[1][0] !== undefined" class="text-muted-foreground font-thin leading-none">:</div>
        <div v-if="timeArr[2][0] !== undefined" :data-size="props.size" :key="lengthArr[2]" :class="[
          'flex flex-col items-center overflow-y-hidden leading-none',
          `w-[${lengthArr[2]}ch]`,
          'data-[size=xs]:h-[1rem]   data-[size=xs]:leading-[1rem]',
          'data-[size=sm]:h-[2rem]   data-[size=sm]:leading-[2rem]',
        ...
        <div :data-size="props.size" :key="lengthArr[3]" :class="[
          'flex flex-col items-center overflow-y-hidden leading-none',
          `w-[${lengthArr[3]}ch]`,
          'data-[size=xs]:h-[1rem]   data-[size=xs]:leading-[1rem]',
          'data-[size=sm]:h-[2rem]   data-[size=sm]:leading-[2rem]',
          ...
          <div class=" animate-flip-clock" v-for="(timeStr, index) in timeArr[3]" :key="`${index}-${timeStr}`">
            {{ timeStr }}
          </div>
        </div>
        <div v-if="labelRight !== ''" :data-size="props.size" :class="[
          'text-muted-foreground font-thin leading-none',
          'data-[size=xs]:ml-1',
          'data-[size=sm]:ml-1',
          'data-[size=md]:ml-3',
...
function updateClock() {
  ...
  if (didChange) {
    for (let i = 0; i < timeStrs.length; i++) {
      ...
      timeArr.value[i].pop();
      lengthArr.value[i] = timeStrs[i] ? timeStrs[i].length : 0;
    }
  }
}
